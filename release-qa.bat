@echo off
REM ============================================================
REM  Pasha QA - Standalone build assembler + deploy
REM
...
REM       public                    <- public assets, incl. launch.html and
REM                                    hub-launch.html (the static pages served
REM                                    at /launch and /hub-launch)
...
REM  5. Extracts it into /var/www/pasha-qa/ and restarts pasha-qa.service
REM
...
set "REMOTE_DIR=/var/www/pasha-qa"
set "SERVICE=pasha-qa.service"
...
REM The /launch event page is a static file, not an app route, so nothing in
REM the Next build fails if it goes missing - it would just 404 on the night.
REM Fail here instead, while there is still time to fix it.
if not exist "%STAGE%\public\launch.html" (
    set "ERR=public\launch.html missing from the staged build - /launch would 404."
    goto :fail
)
if not exist "%STAGE%\public\hub-launch.html" (
    set "ERR=public\hub-launch.html missing from the staged build - /hub-launch would 404."
    goto :fail
)
echo     %GREEN%launch.html + hub-launch.html staged (serve /launch, /hub-launch)%RESET%

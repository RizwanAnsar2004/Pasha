 ...
 * group, stored in submissions.answers.awards) into `startup_awards` rows tagged
 * `source='submission'`. Same rules as syncAwardsFromText — replaces only prior
 * submission rows, never touches admin 'manual' rows, and skips titles that
 * already exist as manual entries.
 *
 * Best-effort. If the `description` column isn't present yet, the insert is
 * retried without it so approved awards are never lost.
 */

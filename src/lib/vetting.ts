 ...
 * Mirrors the rubric in PSEC-Vetting-Framework-v0.2.pdf.
 *
 * 10 hard gates (fail any → excluded)
 * 10 scoring dimensions (0-5 each, total 0-50)
 * Tier thresholds: Featured ≥35, Listed 25-34, Watchlist 15-24, else Excluded
 */
...
export type VettingInput = {
  ...
  // stage / depth
  stage?: "ideation" | "dev_launch" | "early" | "growth";
  ...
  // at the call site (count where gender === "female"). Kept as a number
  // input here so the caller can pass any derivation in.
...
export function scoreVetting(input: VettingInput): VettingResult {
  ...
  const stageMap: Record<string, { score: number; reason: string }> = {
    ideation: { score: 1, reason: "Ideation stage" },
    dev_launch: { score: 2, reason: "Building MVP" },
    early: { score: 4, reason: "Early revenue" },
    growth: { score: 5, reason: "Scaling" },
  };
  ...
  // 6. Revenue model clarity — having a stage + revenue band
  const revClarity =
    input.stage && input.stage !== "ideation" ? 4 : input.stage ? 2 : 0;
  breakdown.push({
  ...
  // removed from the form (revenue_band is now the single funnel signal).
  // Re-derive from revenue_band: any revenue at all → 3, $50K+ → 4, $1M+ → 5.
  const acqScore = (() => {
